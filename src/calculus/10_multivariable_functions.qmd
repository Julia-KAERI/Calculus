...
::: {#thm-CAL_differentiability_of_multi_variable_functions}
...
이며 각각의 성분도 동차 다항함수이므로 연속이다. 따라서 동차 다항함수는 일급함수이다.
...
좌표평면의 원점 근방에서 정의된 함수 $f(x,\,y)$ 에 대하여 $D_1 f(O)=1,\, D_2 f(O)=-2$ 라 하자. 이 때 함수값의 변화가 거의 없는 방향은 어느 방향인가?
...
$$
f(x,\,y) = F(x^2+y^2)-F(xy),\qquad \qquad F'(s) = e^{-(s-1)^2}
$$
...
## 10.7 부록 : 일급함수, 열린 집합, 닫힌 집합, 유계인 집합

### 기본연습문제

<div class="border" style="background-color:#F0FFFF  ;padding:5px;">

::: {#exr-CAL_10_7_1_B02_1}

#### SNU 기본연습문제 10.7.1.2.1
...
::: {#exr-CAL_10_7_1_B02_2}

#### SNU 기본연습문제 10.7.1.2.2
...
($1$) 아르키메데스의 원리, 즉 임의의 두 양의 실수 $0<a<b$ 에 대해 $an>b$ 를 만족하는 자연수 $n$ 이 존재한다는 것을 우리는 앞서 배웠다. 이를 이용하여 임의의 두 실수 $a<b$ 에 대해 $a<p<b$ 를 만족하는 유리수 $p$ 가 존재함을 보이자. (@Rudin1976, theorm 1.20 참고)
